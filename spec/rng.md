...
## Goal
Draw tarot cards with **uniform probability** across all cards, with a **50/50** chance of reversal, using **ANU QRNG** as the default source of entropy. If ANU is unavailable, fall back to **random.org**, and if that fails, fall back to a **slot-machine click** mechanic.
...
- `reversed = (card >= 78)` (exact 50/50 if `card` is uniform)
